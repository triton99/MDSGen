...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., num_patches=None):
        ...
        self.rel_pos_bias = RelativePositionBias(
            window_size=[102, 8], num_heads=num_heads)
...
class TimestepEmbedder(nn.Module):
    ...
    def __init__(self, hidden_size, frequency_embedding_size=204):
        super().__init__()
...
class MDSGen(nn.Module):
    ...
    def __init__(
        ...
        self.mlp_feat = nn.Sequential(
            nn.Linear(in_features=512, out_features=hidden_size, bias=True),
            nn.GELU(),
    ...
    def initialize_weights(self):
        ...
        decoder_pos_embed = get_2d_sincos_pos_embed(
            self.decoder_pos_embed.shape[-1], grid_size_1, grid_size_2)
        ...
        # Zero-out adaLN modulation layers in XMDPT blocks:
        ...
        nn.init.normal_(self.mlp_feat[0].weight, std=0.02)
        nn.init.normal_(self.conv1d_feat[0].weight, std=0.02)
    ...
    def random_masking(self, x, mask_ratio):
        ...
        N_h, L, D_h = x.shape  # batch, length, dim
        L_h = int((L * 2) / self.x_embedder.img_size[1])
        len_keep = int(int(L_h * (1 - mask_ratio)) * (self.x_embedder.img_size[1] / 2)) 

        noise = torch.rand(N_h, L_h, device=x.device)  # noise in [0, 1]

        matrix_num = torch.arange(L, device=x.device).view(L_h, -1).repeat(N_h, 1, 1)

        # sort noise for each sample
        # ascend: small is keep, large is remove
        ids_shuffle_col = torch.argsort(noise, dim=1)
        ids_shuffle = torch.gather(matrix_num, dim=1, index=ids_shuffle_col.unsqueeze(-1).repeat(1, 1, matrix_num.shape[-1])).flatten(1)
        ...
        x_masked = torch.gather(
            x, dim=1, index=ids_keep.unsqueeze(-1).repeat(1, 1, D_h))

        # generate the binary mask: 0 is keep, 1 is remove
        mask = torch.ones([N_h, L], device=x.device)
        mask[:, :len_keep] = 0
    ...
    def forward(self, x, t, enable_mask=False, mix_video_feat=None, prob=0.1):
        """
        Forward pass of XMDPT.
        ...
        with autocast(dtype=torch.bfloat16):
            ...
            y = self.mlp_feat(mix_video_feat)
            y = self.conv1d_feat(y).squeeze(1)
    ...
    def forward_with_cfg(self, x, t, cfg_scale=None, diffusion_steps=1000, scale_pow=4.0, mix_video_feat=None):
        """
        Forward pass of XMDPT, but also batches the unconditional forward pass for classifier-free guidance.
        """
        # https://github.com/openai/glide-text2im/blob/main/notebooks/text2im.ipynb
        ### MADPIT inference with cfg
        if cfg_scale is not None:
            uncond_eps = self.forward(x, t, False, mix_video_feat, 1)
            cond_eps = self.forward(x, t, False, mix_video_feat, 0)
...
def get_2d_sincos_pos_embed(embed_dim, grid_size_1, grid_size_2, cls_token=False, extra_tokens=0):
    """
...
def MDSGen_B_2(**kwargs):
    return MDSGen(depth=12, hidden_size=768, patch_size=(2, 2), num_heads=12, **kwargs)
